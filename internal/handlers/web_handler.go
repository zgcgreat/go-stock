...
// FetchAiModelInfo 获取单个AI模型的最大Token信息（与桌面端 App.FetchAiModelInfo 逻辑一致）
func FetchAiModelInfo(c *gin.Context) {
	baseUrl := strings.TrimSpace(c.Query("baseUrl"))
	apiKey := strings.TrimSpace(c.Query("apiKey"))
	modelName := strings.TrimSpace(c.Query("modelName"))

	info := &AiModelInfo{ModelName: modelName}
	...
	if apiKey != "" && baseUrl != "" && modelName != "" {
		...
		resp, err := client.R().SetResult(&detail).Get("/models/" + modelName)
	...
	if info.MaxTokens == 0 {
		if maxTokens := getWebBuiltinModelMaxTokens(modelName); maxTokens > 0 {
...
// getWebBuiltinModelMaxTokens 内置模型最大Token对照表（与桌面端 getBuiltinModelMaxTokens 一致）
func getWebBuiltinModelMaxTokens(modelName string) int {
