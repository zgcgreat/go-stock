...
// GetSession 获取指定 sessionId 的会话消息列表，若 sessionId 为空则获取最新的
func (s *AiAssistantService) GetSession(sessionId string) (*models.AiAssistantSessionResp, error) {
...
// SaveSession 保存会话消息到数据库，若 sessionId 已存在则更新，否则创建新记录
func (s *AiAssistantService) SaveSession(sessionId string, messages []models.AiAssistantMessage) error {
	if len(messages) == 0 {
		return nil
	}
	toSave := messages
...
// GetAiAssistantService 获取AI助手服务实例
func GetAiAssistantService() *AiAssistantService {
