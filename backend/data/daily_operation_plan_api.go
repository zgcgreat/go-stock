...
func (a *DailyOperationPlanApi) GetDailyOperationPlanList(query *models.DailyOperationPlanQuery, userID ...uint) (*models.DailyOperationPlanPageData, error) {
	var list []models.DailyOperationPlan
	var total int64

	q := db.Dao.Model(&models.DailyOperationPlan{})
	if uid := firstUserID(userID); uid > 0 {
...
func (a *DailyOperationPlanApi) GetDailyOperationPlanByID(id uint, userID ...uint) (*models.DailyOperationPlan, error) {
...
func (a *DailyOperationPlanApi) SaveDailyOperationPlan(plan models.DailyOperationPlan, userID ...uint) string {
	// 校验情景/纪律 JSON 字段格式
	if plan.Scenarios != "" {
	...
	updateQ := db.Dao.Model(&models.DailyOperationPlan{}).Where("id = ?", plan.ID)
	if uid := firstUserID(userID); uid > 0 {
		updateQ = updateQ.Where("user_id = ?", uid)
	}
	if err := updateQ.Updates(map[string]any{
...
func (a *DailyOperationPlanApi) DeleteDailyOperationPlan(id uint, userID ...uint) string {
	var plan models.DailyOperationPlan
	q := db.Dao.Where("id = ?", id)
	if uid := firstUserID(userID); uid > 0 {
		q = q.Where("user_id = ?", uid)
...
func (a *DailyOperationPlanApi) UpdateDailyOperationPlanStatus(id uint, status string, userID ...uint) error {
	q := db.Dao.Model(&models.DailyOperationPlan{}).Where("id = ?", id)
	if uid := firstUserID(userID); uid > 0 {
		q = q.Where("user_id = ?", uid)
	}
...
func (a *DailyOperationPlanApi) UpdateDailyOperationPlanAlert(id uint, enableAlert bool, userID ...uint) error {
	q := db.Dao.Model(&models.DailyOperationPlan{}).Where("id = ?", id)
	if uid := firstUserID(userID); uid > 0 {
		q = q.Where("user_id = ?", uid)
	}
	return q.Update("enable_alert", enableAlert).Error
}

// firstUserID 返回第一个 userID，无则返回 0
func firstUserID(userID []uint) uint {
	if len(userID) > 0 {
		return userID[0]
	}
	return 0
